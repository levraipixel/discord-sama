...
resource "aws_lambda_function" "bot" {
  ...
  description      = "Serverless Discord bot (${var.environment})"
...
resource "aws_iam_role_policy" "lambda_dynamodb_reminders" {
  name = "${local.name_prefix}-lambda-dynamodb-reminders"
...
resource "aws_scheduler_schedule" "check_reminders" {
  name = "${local.name_prefix}-check-reminders"
...
resource "aws_iam_role_policy" "lambda_self_invoke" {
  name = "${local.name_prefix}-lambda-self-invoke"
  role = aws_iam_role.lambda_exec.id
...
resource "aws_apigatewayv2_api" "bot" {
  ...
  protocol_type = "HTTP"
  description   = "Discord interactions endpoint for ${local.name_prefix}"
}
